...
# Load fitted models
fit_g1 <- readRDS("../fits/cumulative_influence/fit_G1_dpi_cum.rds")
...
whale_index_to_id_g1 <- setNames(whale_id_map_g1$WhaleID, whale_id_map_g1$WhaleIndex)
...
omega_plot_df_g1 <- fit_g1$draws("omega") %>%
  spread_draws(omega[i]) %>%
  mutate(Whale = whale_index_to_id_g1[as.character(i)])  # map index -> ID
...
ggplot(omega_plot_df_g1, aes(x = omega, fill = Whale)) +
...
ggsave("omega_post_G1.png", width = 10, height = 10, units = "in", dpi = 300)
...
incoming_influence_g1 <- A_summary_g1 %>%
  group_by(i) %>%
  summarise(total_influence_g1 = sum(mean_A)) %>%
  mutate(Whale = whale_index_to_id_g1[as.character(i)])

plot_overlay_g1 <- omega_summary_g1 %>%
  left_join(incoming_influence_g1, by = c("i", "Whale"))

# Added colour by whale, and icon by subgroup
plot_overlay_g1_col <- plot_overlay_g1 %>%
  left_join(g1_df %>% select(WhaleID, Source_subgroup) %>% distinct(), 
            by = c("Whale" = "WhaleID"))

ggplot(plot_overlay_g1_col, aes(x = total_influence_g1, y = omega_median)) +
  geom_point(aes(color = as.factor(Source_subgroup))) +
  geom_text_repel(aes(label = Whale), max.overlaps = 10, size = 4) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray50") +
  labs(
    title = "Whales' Baseline Calling Rate vs. Social Influence (Group G1)",
...
ggsave("omega_influce_overlay_G1.png", width = 10, height = 10, units = "in", dpi = 300)
...
N <- max(stan_data_g1$caller)
A_mat <- matrix(0, N, N)
for (row in 1:nrow(A_matrix)) {
  A_mat[A_matrix$i[row], A_matrix$j[row]] <- A_matrix$mean_A[row]
}

lambda_mean <- summarise_draws(fit_g1$draws("lambda"))$mean

lambda <- lambda_mean

influence_traces <- list()

for (b in long_bouts_g1[1:10]) {
  sub_bout <- g1_df %>%
    filter(bout == b) %>%
    arrange(StartTime) %>%
...
call_lines_df <- g1_df %>%
  filter(bout %in% long_bouts_g1[1:10]) %>%
...
edge_df_g1 <- A_summary_g1 %>%
  mutate(
    from_whale = whale_index_to_id_g1[as.character(j)],
    to_whale   = whale_index_to_id_g1[as.character(i)]
  ) %>%
  select(from = from_whale, to = to_whale, mean_A)
# Optional threshold

# Create graph object
g1_influence <- graph_from_data_frame(edge_df_g1, directed = TRUE)


# Compute strength (weighted degree)
V(g1_influence)$in_strength  <- strength(g1_influence, mode = "in", weights = E(g1_influence)$mean_A)
V(g1_influence)$out_strength <- strength(g1_influence, mode = "out", weights = E(g1_influence)$mean_A)

# Centrality
V(g1_influence)$eigen_centrality <- eigen_centrality(g1_influence, weights = E(g1_influence)$mean_A)$vector
V(g1_influence)$betweenness <- betweenness(g1_influence, directed = TRUE)


ggraph(g1_influence, layout = "nicely") +
...
ggraph(layout) +
  ...
  theme_void() +
  labs(
    title = "Whale Influence Network (G1)",
    fill = "Most Influential",
    size = "Total Influence Received"
  )
# save as svg to fix in inkscape
ggsave("dbw_infl_whale_network_G1.svg", width = 10, height = 10, units = "in", dpi = 300) 


#### ----------- Compare posterior and actual delays G1 ---------- ####
...
pair_lookup <- tibble(
  k = 1:length(stan_data_g1$pair_i),
...
# Join means into g1_data
g1_joined <- g1_data %>%
  left_join(omega_mean, by = c("WhaleIndex" = "i"))

# We'll build this loop-style (you can vectorize later)
mu_list <- vector("numeric", length = nrow(g1_joined))

for (n in seq_len(nrow(g1_joined))) {
  row_n <- g1_joined[n, ]
  i <- row_n$WhaleIndex
  t_n <- row_n$StartTime
...
g1_joined$PredictedDelay <- mu_list


ggplot(g1_joined, aes(x = ObservedDelay, y = PredictedDelay)) +
...
ggplot(plot_overlay_g2, aes(x = total_influence_g2, y = omega_median, label = Whale)) +
  geom_point(size = 3) +
  geom_text_repel(max.overlaps = 10, size = 4) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray50") +
  labs(
    title = "Whales' Baseline Calling Rate vs. Social Influence (Group g2)",
    x = expression("Total Incoming Influence ("*sum(A[ij])*")"),
    y = expression("Median "*omega[i]*" (Baseline Calling Rate)")
  ) +
  theme_minimal(base_size = 14)
...
levels <- levels(factor(g1_df$WhaleID))
data.frame(WhaleIndex = seq_along(levels), WhaleID = levels)


# Assume: pair_i and pair_j are already defined from stan_data_g1
# Pull WhaleID labels into named vectors (not join twice)
from_labels <- whale_id_map_g1 %>% rename(from = WhaleID)
to_labels   <- whale_id_map_g1 %>% rename(to = WhaleID)
